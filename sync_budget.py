...
Updates DCR Ledger.xlsx → Transactions sheet.
...
FIRST-TIME SETUP:
  ...
  3. Open DCR Ledger.xlsx — all transactions will be there
"""
...
def resolve_xlsx_path() -> Path:
    """
    Locates the active DCR Ledger spreadsheet file.
    Searches candidates:
      - 'DCR Ledger.xlsx'
      - 'DCR_Ledger.xlsx'
      - 'DCR_Budget_Tracker.xlsx'
      - 'DCR Budget Tracker.xlsx'
    across:
    ...
    # Fallback: search for any non-temporary .xlsx containing ledger or budget
    for d in search_dirs:
        if d.is_dir():
            for p in d.glob("*.xlsx"):
                if not p.name.startswith("~$") and not p.name.startswith("._") and "backup" not in p.name.lower():
                    if any(k in p.name.lower() for k in ["ledger", "budget"]):
                        return p.resolve()

    # Default fallback
    if (BASE_DIR.parent / "DCR Ledger.xlsx").exists():
        return (BASE_DIR.parent / "DCR Ledger.xlsx").resolve()
    return (BASE_DIR / "DCR Ledger.xlsx").resolve()


def resolve_monzo_dir() -> Path:
    search_dirs = [
        BASE_DIR / "Monzo",
        BASE_DIR.parent / "Monzo",
        Path.cwd() / "Monzo",
    ]
    for d in search_dirs:
        if d.is_dir():
            return d.resolve()
    default_dir = BASE_DIR / "Monzo"
    default_dir.mkdir(parents=True, exist_ok=True)
    return default_dir.resolve()


XLSX_PATH     = resolve_xlsx_path()
MONZO_DIR     = resolve_monzo_dir()
...
<div style="max-width:700px;margin:0 auto;padding:24px">
  ...
  <div style="text-align:center;padding:16px;color:#334155;font-size:11px">
    ...
    Open DCR Ledger.xlsx to review transactions in detail.
  </div>
</div></body></html>"""
...
<div style="max-width:640px;margin:0 auto;padding:24px">
  ...
  <div style="text-align:center;padding:14px;color:#334155;font-size:11px">DCR Budget System · Open DCR Ledger.xlsx for full detail</div>
</div></body></html>"""
...
def main():
    ...
    print("\n✅ Sync complete.")
    if added > 0:
        print(f"   Open DCR Ledger.xlsx to review {added} new transactions.")
