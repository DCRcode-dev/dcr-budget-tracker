...
function buildConfigTab_(sheet, ss) {
  ...
  const configRows = [
    ...
    ["ANNUAL_SALARY_GROSS", "Annual gross salary (USD)", 150000, "$#,##0.00", true, "$150k gross PR employee compensation."],
    ["BIWEEKLY_SALARY_NET", "Bi-weekly net paycheck (USD)", 4034.15, "$#,##0.00", true, "Net after FICA (7.65%) and PR tax (~22.4%)."],
    ["MONTHLY_SALARY", "Monthly net salary (USD · 2 checks)", 8068.30, "$#,##0.00", true, "2 bi-weekly checks ($4,034.15 * 2). Annualized avg is $8,740.66."],
    ["SALARY_START_DATE", "Salary start date", "09/09/2026", "MM/DD/YYYY", true, "Started Sep 9, 2026."],
    ["FIRST_PAYCHECK_EST", "First paycheck est. (Prorated Sep 9–18)", 3227.32, "$#,##0.00", true, "8 work days proration paid in Banco Popular."],
