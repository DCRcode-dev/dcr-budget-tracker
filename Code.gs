...
function buildConfigTab_(sheet, ss) {
  ...
  const configRows = [
    ...
    ["MORTGAGE_PAYMENT", "Mortgage monthly payment (USD)", 0, "$#,##0.00", true, "Years 1–11, includes FHA MIP."],
    ...
    ["ANNUAL_TOTAL_INCOME", "Annual total income (USD)", 150000, "$#,##0.00", true, "$100k taxable salary + $50k non-taxable donation."],
    ["ANNUAL_TAXABLE_SALARY", "Annual taxable salary (USD)", 100000, "$#,##0.00", true, "Subject to PR Hacienda tax and FICA."],
    ["ANNUAL_NONTAX_DONATION", "Annual non-taxable donation (USD)", 50000, "$#,##0.00", true, "100% tax-free basis ($1,923.08 bi-weekly)."],
    ["BIWEEKLY_TOTAL_NET", "Bi-weekly total net income (USD)", 4784.15, "$#,##0.00", true, "$2,861.07 taxable net + $1,923.08 non-taxable."],
    ["MONTHLY_SALARY", "Monthly net income (USD · 2 checks)", 9568.30, "$#,##0.00", true, "2 bi-weekly checks ($4,784.15 * 2). Annualized avg is $10,365.66."],
    ["SALARY_START_DATE", "Salary start date", "09/09/2026", "MM/DD/YYYY", true, "Started Sep 9, 2026."],
    ["FIRST_PAYCHECK_EST", "First paycheck est. (Paid Sep 25)", 3827.31, "$#,##0.00", true, "Paid Fri Sep 25 (two Fridays after first week)."],
...
function buildAccountsTab_(sheet) {
  ...
  const seedRows = [
    ...
    ["Merrill Lynch Investment", "Merrill Lynch", "Investment", "USD", 0, '=IF(D4="GBP", E4*GBP_USD_RATE, E4)', "", "Active", false, "SimpleFIN", ""],
  ...
  try {
    ...
    const payload = {
      ...
      net_worth: (() => {
        ...
        const merrillBal = merAcc ? (parseFloat(merAcc[5] || merAcc[4] || 0)) : 0;
        let cardDebt = 0;
